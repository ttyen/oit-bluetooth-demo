		...
		if (cmdlist[0] == "GETFILE"):
			...
			filearray=""
				...
				filearray=filearray+'{"name":"'+str(e)+'"},'
            		#for e in onlyfiles:
			#	filearray=filearray+'{"'+str(e)+'"},'
			filearray=filearray[0:-1]
			filejson='{"typeinfo": "listfile","filename":['+filearray+']}'
			print filejson
			print(json.dumps(filejson));
            		#client_sock.sendall(json.dumps(filejson))
            		client_sock.sendall(filejson)
            		#client_sock.sendall(json.dumps({"typeinfo": "listfile","filename":[{"name":"E79987.pdf","name":"aaaa.txt"}]}))
            		#client_sock.sendall(''.join(str(e) for e in onlyfiles))
except IOError:
    pass
